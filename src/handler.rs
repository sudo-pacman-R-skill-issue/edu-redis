...
pub fn extract_value(resp: RespOrig) -> Option<Bytes> {
    match resp {
...
impl RespOrig {
    fn handler(&self) -> Bytes {
        match self {
            RespOrig::String(bytes) => todo!(),
            RespOrig::Error(bytes) => todo!(),
            RespOrig::Int(_) => todo!(),
            RespOrig::Array(resp_origs) => {
                todo!()
            }
            RespOrig::NullArray => todo!(),
            RespOrig::NullBulkString => todo!(),
        }
    }
}
